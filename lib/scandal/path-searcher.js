      ...
      reader.on('data', (function(_this) {
        return function(chunk) {
          ...
          for (j = 0, len1 = lines.length; j < len1; j++) {
            ...
            Object.keys(wordMap).forEach(function (workKey){

              if(self.mode == "one" && wordMap[workKey]["matched"]){
                return ;
              }

              if (!wordMap[workKey]["foundFileList"][filePath]  ){
                wordMap[workKey]["foundFileList"][filePath] = [];
              }

              var regex = new RegExp( "\\b" + workKey  + "\\b", 'g');
              ...
              if (lineMatches != null) {
                ...
                for (k = 0, len2 = lineMatches.length; k < len2; k++) {
                  match = lineMatches[k];
                  matches.push(match);
                    wordMap[workKey]["foundFileList"][filePath].push(match);
                  if(self.mode == "one"){
                    wordMap[workKey]["matched"] = true;
                  }
                }
              }
